...
type Scenario = "ex_late" | "abroad_weak" | "business_weak" | "buy_uncovered" | null;
...
type Flags = {
  ...
  housingRisk: boolean; legalRisk: boolean; physicalRisk: boolean; relationshipRisk: boolean; urgentHits: number; poeticHits: number; supportCount: number; missingCount: number; abroadMissing: number;
};
...
function detectFlags(t: string): Flags {
  ...
  const abroad = hasAny(t, T.viaggio), quitJob = hasAny(t, ["mi licenzio", "lascio il lavoro", "mollo il lavoro", "mollo tutto"]), business = hasAny(t, ["apro", "bar", "studio", "agenzia", "startup", "locale", "azienda"]), purchase = hasAny(t, ["compro", "moto", "macchina", "auto", "van", "casa", "acquisto"]), exContact = hasAny(t, ["le scrivo", "gli scrivo", "le riscrivo", "gli riscrivo", " ex"]), reset = hasAny(t, ["cambio vita", "nuova vita", "reset", "sparisco", "ricomincio da zero"]), creative = hasAny(t, ["studio creativo", "brand", "progetto", "collettivo", "agenzia creativa"]), immediate = hasAny(t, P.urgent);
  const debt = hasAny(t, ["debito", "debiti", "prestito", "rata", "mutuo", "mi indebito"]), housingRisk = hasAny(t, ["lascio casa", "senza casa", "mollo casa"]), legalRisk = hasAny(t, ["documenti falsi", "evasione", "truffa", "reato"]), physicalRisk = hasAny(t, ["pericoloso", "senza casco", "senza patente", "rischio fisico"]), relationshipRisk = exContact || hasAny(t, ["sparisco senza dire nulla", "tradisco"]);
  return { noMoney, noPlan, noSupport, noLanguage, noDocuments, noWork, noClients, noExperience, hasBudget, hasPlan, hasSupport, hasLanguage, hasDocuments, hasJob, hasClients, hasTimeline, gradual, admitted, immediate, abroad, quitJob, business, purchase, exContact, reset, creative, debt, housingRisk, legalRisk, physicalRisk, relationshipRisk, urgentHits: countHits(t, P.urgent), poeticHits: countHits(t, P.poetic), supportCount: [hasBudget, hasPlan, hasSupport, hasLanguage, hasDocuments, hasJob, hasClients, hasTimeline, gradual, admitted].filter(Boolean).length, missingCount: [noMoney, noPlan, noSupport, noLanguage, noDocuments, noWork, noClients, noExperience].filter(Boolean).length, abroadMissing: [noMoney, noPlan, noSupport, noLanguage, noDocuments, noWork].filter(Boolean).length };
}
...
function buildFinalLine(theme: Theme, driver: Driver, s: Scenario, seed: number) {
...
export function analyzePlan(input: string): AnalysisResult {
  ...
  const dissociazione = clamp(18 + objectiveScale * 9 + f.missingCount * 7 + Number(scenario === "abroad_weak") * 22 + Number(scenario === "business_weak") * 18 + Number(scenario === "ex_late") * 20 + Number(f.noPlan) * 8 + Number(f.noMoney) * 6 - f.supportCount * 5, 1, 100);
  const impulsivita = clamp(12 + f.urgentHits * 16 + Number(f.immediate) * 10 + Number(f.quitJob) * 8 + punctuation * 3 + steps * 2 - Number(f.gradual) * 12 - Number(f.hasTimeline) * 5 - Number(f.hasPlan) * 4, 1, 100);
  const dannoPratico = clamp(12 + f.missingCount * 6 + Number(f.noMoney) * 16 + Number(f.debt) * 14 + Number(f.quitJob) * 14 + Number(f.noWork) * 10 + Number(f.noDocuments) * 10 + Number(f.noClients) * 8 + Number(f.housingRisk) * 15 + Number(f.legalRisk) * 20 + Number(f.physicalRisk) * 18 + Number(f.relationshipRisk) * 10 + Number(scenario === "abroad_weak") * 16 + Number(scenario === "business_weak") * 14 + Number(scenario === "ex_late") * 8 - f.supportCount * 3, 1, 100);
  const poetico = clamp(8 + f.poeticHits * 15 + Number(f.reset) * 10 + Number(f.abroad) * 8 + Number(f.exContact) * 10 + Number(driver === "romanticismo") * 10 + Number(driver === "fuga") * 7 + Number(driver === "reset") * 8 + Number(scenario === "abroad_weak") * 6 + Number(scenario === "business_weak") * 4, 1, 100);
